TRANSLATIONS = {
    'id': {
        # Header
        'desc_app': 'Kelola Keuanganmu dengan mudah, kapan saja, di mana saja.',

        # Navbar
        'nav_home': 'Beranda',
        'nav_report': 'Laporan',
        'nav_scan': 'Scan Struk',
        'nav_sharing': 'Grup',
        'nav_groups': 'Grup',
        'nav_settings': 'Pengaturan',
        'nav_logout': 'Keluar',
        'nav_transaction': 'Transaksi',
        'split_bill': 'Split Bill',

        # Index
        'welcome': 'Selamat Datang',
        'total_income': 'Total Pemasukan',
        'total_expense': 'Total Pengeluaran',
        'balance': 'Saldo',
        'add_transaction': 'Tambah Transaksi',
        'transaction_history': 'Riwayat Transaksi',
        'date': 'Tanggal',
        'title': 'Judul',
        'category': 'Kategori',
        'type': 'Tipe',
        'amount': 'Jumlah',
        'actions': 'Aksi',
        'action': 'Aksi',
        'income': 'Pemasukan',
        'expense': 'Pengeluaran',
        'edit': 'Edit',
        'delete': 'Hapus',
        'no_transaction': 'Belum ada transaksi. Tambahkan yang pertama!',
        'confirm_delete': 'Yakin mau hapus transaksi ini?',

        # Form transaksi
        'add_title': 'Tambah Transaksi',
        'edit_title': 'Edit Transaksi',
        'form_title': 'Judul Transaksi',
        'form_amount': 'Jumlah (Rp)',
        'form_date': 'Tanggal',
        'form_type': 'Tipe',
        'form_category': 'Kategori',
        'form_note': 'Catatan (opsional)',
        'btn_save': 'Simpan',
        'btn_cancel': 'Batal',
        'btn_save_changes': 'Simpan Perubahan',
        'transaction_title': 'Judul Transaksi',
        'transaction_title_placeholder': 'Contoh: Makan siang, Bensin',
        'note': 'Catatan',
        'note_placeholder': 'Contoh: Untuk beli kopi',
        'amount_placeholder': 'Contoh: 50000',
        'optional': 'opsional',

        # Kategori
        'food': 'Makanan',
        'transport': 'Transport',
        'shopping': 'Belanja',
        'entertainment': 'Hiburan',
        'health': 'Kesehatan',
        'education': 'Pendidikan',
        'salary': 'Gaji',
        'other': 'Lainnya',

        # Scan
        'scan_title': 'Scan Struk Belanja',
        'scan_desc': 'Upload foto struk, sistem otomatis membaca data transaksi.',
        'upload_receipt': 'Upload Foto Struk',
        'scan_receipt': 'Scan Struk',
        'scan_btn': 'Scan Struk',
        'scan_result': 'Hasil Scan — Konfirmasi Data',
        'rescan': 'Scan Ulang',
        'save_transaction': 'Simpan Transaksi',
        'detected_items': 'Item yang terdeteksi',

        # Laporan
        'report_title': 'Laporan Keuangan',
        'expense_by_cat': 'Pengeluaran per Kategori',
        'income_vs_expense': 'Pemasukan vs Pengeluaran',
        'no_data': 'Belum ada data.',
        'no_expense_data': 'Belum ada data pengeluaran.',
        'no_transaction_data': 'Belum ada data transaksi.',
        'report': 'Laporan',

        # Budget
        'budget_this_day': 'Hari Ini',
        'budget_this_week': 'Minggu Ini',
        'budget_this_month': 'Bulan Ini',
        'budget_remaining': 'Sisa',
        'budget_over': 'Melebihi batas pengeluaran',
        'budget_empty': 'Batas Pengeluaran Habis!',
        'budget_warning': 'Peringatan batas pengeluaran!',
        'set_budget': 'Atur Batas Pengeluaran',
        'save_budget': 'Simpan Batas Pengeluaran',
        'change_budget': 'Ubah Batas Pengeluaran',
        'no_budget_set': 'Belum ada batas pengeluaran yang diatur.',
        'set_budget_now': 'Atur Batas Pengeluaran Sekarang',
        'budget_amount': 'Jumlah Batas Pengeluaran',
        'budget_period': 'Periode',
        'period': 'Periode',
        'daily': 'Harian',
        'weekly': 'Mingguan',
        'monthly': 'Bulanan',
        'save': 'Simpan',
        'manage_budget': 'Kelola Batas Pengeluaran',
        'budget_amount_placeholder': 'Contoh: 500000',
        'remaining': 'Sisa',
        'over_budget': 'Melebihi Batas Pengeluaran',
        'edit_budget': 'Ubah Batas Pengeluaran',

        # Grup / Sharing
        'mode_sharing': 'Mode Sharing',
        'create_group': 'Buat Grup',
        'join_group': 'Gabung Grup',
        'your_groups': 'Grup Kamu',
        'group_name': 'Nama Grup',
        'group_code': 'Kode Grup',
        'open_group': 'Buka',
        'members': 'Anggota',
        'invite_code': 'Kode Undangan',
        'desc_code': 'Masukkan kode unik dari temanmu',
        'no_groups': 'Belum ada grup. Buat atau gabung grup dulu!',
        'code': 'Kode',
        'member': 'Anggota',
        'group': 'Grup',
        'group_name_placeholder': 'Contoh: Kos Bareng, Kel 12',
        'group_code_placeholder': 'Contoh: A1B2C3D4',

        # Grup transaksi
        'code_invitation': 'Kode Undangan',
        'desc_code_invitation': 'Bagikan kode unik ini ke temanmu untuk bergabung.',
        'no_transactions': 'Belum ada transaksi di grup ini.',
        'group_balance': 'Saldo Grup',
        'fund_desc': 'Masuk ke Dana (opsional)',
        'fund_desc_placeholder': 'Contoh: Untuk dana kegiatan',
        'all_funds': 'Semua Dana',
        'no_fund': 'Tanpa Dana',
        'all_types': 'Semua Tipe',
        'all_members': 'Semua Anggota',
        'reset_filters': 'Atur ulang filter',
        'riwayat_transaksi': 'Riwayat Transaksi',
        'fund_income': 'Pemasukan Dana',
        'fund_expense': 'Pengeluaran Dana',
        'fund_balance': 'Saldo Dana',
        'fund_target': 'Target Dana',
        'fund_target_placeholder': 'Contoh: 1000000',
        'fund_progress': 'Progres Dana',
        'no_funds': 'Belum ada dana. Tambahkan pos keuangan dulu!',
        'activity_log_desc': 'Riwayat aktivitas anggota',
        'no_activities': 'Belum ada aktivitas.',
        'showing_last_activities': 'Menampilkan 20 aktivitas terakhir',
        'activity_log': 'Log Aktivitas',
        'fund_name': 'Nama Dana / Pos Keuangan',
        'fund_name_placeholder': 'Contoh: Dana Kegiatan, Dana Operasional',
        'add_fund': 'Tambah Dana',
        'edit_fund': 'Edit Dana',
        'delete_fund': 'Hapus Dana',
        'confirm_delete_fund': 'Yakin mau hapus dana ini? Semua transaksi yang terkait akan dipindahkan ke "Tanpa Dana".',
        'funds': 'Dana',
        'of_target': 'dari target',

        # Tabel laporan
        'percentage': 'Persentase',
        'share': 'Porsi',

        # Filter bulan
        'all_months': 'Semua Bulan',
        'january': 'Januari',
        'february': 'Februari',
        'march': 'Maret',
        'april': 'April',
        'may': 'Mei',
        'june': 'Juni',
        'july': 'Juli',
        'august': 'Agustus',
        'september': 'September',
        'october': 'Oktober',
        'november': 'November',
        'december': 'Desember',
        'all_years': 'Semua Tahun',

        # Grup report
        'expense_by_member': 'Pengeluaran per Anggota',
        'expense_by_fund': 'Pengeluaran per Dana',
        'member_contribution_summary': 'Ringkasan Kontribusi Anggota',
        'fund_summary': 'Ringkasan per Dana / Pos Keuangan',

        # Edit transaksi grup
        'edit_transaction': 'Edit Transaksi Grup',
        'edited_by': 'Diedit oleh',
        'cancel': 'Batal',
        'save_changes': 'Simpan Perubahan',

        # Split bill
        'logout': 'Keluar',
        'from': 'dari',
        'paid': 'Lunas',
        'not_paid': 'Belum Lunas',
        'mark_paid': 'Tandai Lunas',
        'confirm_delete_bill': 'Yakin mau hapus split tagihan ini?',
        'delete_bill': 'Hapus Split Tagihan',
        'custom_amount_placeholder': 'Jumlah (Rp)',
        'all_paid': 'Semua sudah lunas',
        'pct_paid': 'Persentase lunas',
        'total_paid': 'Total lunas',
        'total_amount': 'Total tagihan',
        'bill_details': 'Detail Tagihan',
        'split_bill_desc': 'Buat tagihan yang dibagi ke beberapa anggota grup.',
        'bill_title': 'Judul Tagihan',
        'bill_title_placeholder': 'Contoh: Makan malam, Belanja bulanan',
        'total_bill': 'Total Tagihan (Rp)',
        'total_bill_placeholder': 'Contoh: 150000',
        'split_method': 'Metode Pembagian',
        'equal_split': 'Bagi Rata',
        'custom_split': 'Kustom',
        'select_members': 'Pilih Anggota',
        'create_split': 'Buat Split',
        'split_bills': 'Daftar Split Tagihan',
        'no_split_bills': 'Belum ada split tagihan.',
        'created_by': 'Dibuat oleh',
        'of': 'dari',
        'transactions': 'transaksi',

        # Settings
        'settings_title': 'Pengaturan Akun',
        'profile': 'Profil',
        'preferences': 'Preferensi',
        'security': 'Keamanan',
        'export_data': 'Ekspor Data',
        'login_history': 'Riwayat Login',
        'delete_account': 'Hapus Akun',
        'full_name': 'Nama Lengkap',
        'email': 'Email',
        'profile_photo': 'Foto Profil (JPG/PNG, maks 2MB)',
        'save_profile': 'Simpan Profil',
        'language': 'Bahasa',
        'theme': 'Tema',
        'light_mode': '☀️ Light Mode',
        'dark_mode': '🌙 Dark Mode',
        'timezone': 'Zona Waktu',
        'notification': 'Notifikasi',
        'notif_popup_label': 'Tampilkan notifikasi popup di aplikasi',
        'save_preferences': 'Simpan Preferensi',
        'change_password': 'Ganti Password',
        'current_password': 'Password Saat Ini',
        'new_password': 'Password Baru',
        'confirm_password': 'Konfirmasi Password Baru',
        'joined': 'Bergabung',
        'desc_profile': 'Ubah informasi profil kamu',
        'desc_preferences': 'Sesuaikan tampilan dan bahasa aplikasi',
        'desc_security': 'Gunakan password yang kuat dan unik',
        'desc_export': 'Download semua data transaksi pribadimu',
        'desc_login_history': '5 login terakhir di akunmu',
        'desc_delete_account': 'Tindakan ini tidak bisa dibatalkan. Semua data transaksi dan budget akan dihapus permanen.',
        'form_delete': '⚠️ Yang akan dihapus: semua transaksi, budget, dan data akun kamu.',
        'form_preferensi': 'Preferensi berhasil disimpan!',
        'export_csv': 'Ekspor CSV',
        'desc_export_csv': 'Format spreadsheet universal, bisa dibuka di Excel / Google Sheets',
        'export_excel': 'Ekspor Excel',
        'desc_export_excel': 'Format .xlsx dengan warna dan formatting otomatis',
    },

    'en': {
        # Header
        'desc_app': 'Manage your finances easily, anytime, anywhere.',

        # Navbar
        'nav_home': 'Home',
        'nav_report': 'Report',
        'nav_scan': 'Scan Receipt',
        'nav_sharing': 'Groups',
        'nav_groups': 'Groups',
        'nav_settings': 'Settings',
        'nav_logout': 'Logout',
        'nav_transaction': 'Transactions',
        'split_bill': 'Split Bill',

        # Index
        'welcome': 'Welcome',
        'total_income': 'Total Income',
        'total_expense': 'Total Expense',
        'balance': 'Balance',
        'add_transaction': 'Add Transaction',
        'transaction_history': 'Transaction History',
        'date': 'Date',
        'title': 'Title',
        'category': 'Category',
        'type': 'Type',
        'amount': 'Amount',
        'actions': 'Actions',
        'action': 'Action',
        'income': 'Income',
        'expense': 'Expense',
        'edit': 'Edit',
        'delete': 'Delete',
        'no_transaction': 'No transactions yet. Add your first one!',
        'confirm_delete': 'Are you sure you want to delete this transaction?',

        # Form
        'add_title': 'Add Transaction',
        'edit_title': 'Edit Transaction',
        'form_title': 'Transaction Title',
        'form_amount': 'Amount (Rp)',
        'form_date': 'Date',
        'form_type': 'Type',
        'form_category': 'Category',
        'form_note': 'Note (optional)',
        'btn_save': 'Save',
        'btn_cancel': 'Cancel',
        'btn_save_changes': 'Save Changes',
        'transaction_title': 'Transaction Title',
        'transaction_title_placeholder': 'Example: Lunch, Gasoline',
        'note': 'Note',
        'note_placeholder': 'Example: For coffee purchase',
        'amount_placeholder': 'Example: 50000',
        'optional': 'optional',

        # Kategori
        'food': 'Food',
        'transport': 'Transport',
        'shopping': 'Shopping',
        'entertainment': 'Entertainment',
        'health': 'Health',
        'education': 'Education',
        'salary': 'Salary',
        'other': 'Other',

        # Scan
        'scan_title': 'Scan Receipt',
        'scan_desc': 'Upload a receipt photo, the system will automatically read transaction data.',
        'upload_receipt': 'Upload Receipt Photo',
        'scan_receipt': 'Scan Receipt',
        'scan_btn': 'Scan',
        'scan_result': 'Scan Result — Confirm Data',
        'rescan': 'Scan Again',
        'save_transaction': 'Save Transaction',
        'detected_items': 'Detected items',

        # Report
        'report_title': 'Financial Report',
        'expense_by_cat': 'Expense by Category',
        'income_vs_expense': 'Income vs Expense',
        'no_data': 'No data yet.',
        'no_expense_data': 'No expense data yet.',
        'no_transaction_data': 'No transaction data yet.',
        'report': 'Report',

        # Budget
        'budget_this_day': 'Today',
        'budget_this_week': 'This Week',
        'budget_this_month': 'This Month',
        'budget_remaining': 'Remaining',
        'budget_over': 'Over budget',
        'budget_empty': 'Budget Exhausted!',
        'budget_warning': 'Budget Warning!',
        'set_budget': 'Set Budget',
        'save_budget': 'Save Budget',
        'change_budget': 'Edit Budget',
        'no_budget_set': 'No budget set yet.',
        'set_budget_now': 'Set Budget Now',
        'budget_amount': 'Budget Amount',
        'budget_period': 'Period',
        'period': 'Period',
        'daily': 'Daily',
        'weekly': 'Weekly',
        'monthly': 'Monthly',
        'save': 'Save',
        'manage_budget': 'Manage Budget',
        'budget_amount_placeholder': 'Example: 500000',
        'remaining': 'Remaining',
        'over_budget': 'Over Budget',
        'edit_budget': 'Edit Budget',

        # Groups
        'mode_sharing': 'Sharing Mode',
        'create_group': 'Create Group',
        'join_group': 'Join Group',
        'your_groups': 'Your Groups',
        'group_name': 'Group Name',
        'group_code': 'Group Code',
        'open_group': 'Open',
        'members': 'Members',
        'invite_code': 'Invite Code',
        'desc_code': 'Enter the unique code from your friend',
        'no_groups': 'No groups yet. Create or join a group first!',
        'code': 'Code',
        'member': 'Member',
        'group': 'Group',
        'group_name_placeholder': 'Example: Shared Apartment, Class 12',
        'group_code_placeholder': 'Example: A1B2C3D4',

        # Group transactions
        'code_invitation': 'Invitation Code',
        'desc_code_invitation': 'Share this unique code with your friends to join.',
        'no_transactions': 'No transactions in this group yet.',
        'group_balance': 'Group Balance',
        'fund_desc': 'Allocate to Fund (optional)',
        'fund_desc_placeholder': 'Example: For event fund',
        'all_funds': 'All Funds',
        'no_fund': 'No Fund',
        'all_types': 'All Types',
        'all_members': 'All Members',
        'reset_filters': 'Reset filters',
        'riwayat_transaksi': 'Transaction History',
        'fund_income': 'Fund Income',
        'fund_expense': 'Fund Expense',
        'fund_balance': 'Fund Balance',
        'fund_target': 'Fund Target',
        'fund_target_placeholder': 'Example: 1000000',
        'fund_progress': 'Fund Progress',
        'no_funds': 'No funds yet. Add a financial position first!',
        'activity_log_desc': 'Member activity history',
        'no_activities': 'No activities yet.',
        'showing_last_activities': 'Showing last 20 activities',
        'activity_log': 'Activity Log',
        'fund_name': 'Fund / Financial Position Name',
        'fund_name_placeholder': 'Example: Event Fund, Operational Fund',
        'add_fund': 'Add Fund',
        'edit_fund': 'Edit Fund',
        'delete_fund': 'Delete Fund',
        'confirm_delete_fund': 'Are you sure you want to delete this fund? All related transactions will be moved to "No Fund".',
        'funds': 'Funds',
        'of_target': 'of target',

        # Report table
        'percentage': 'Percentage',
        'share': 'Share',

        # Month filter
        'all_months': 'All Months',
        'january': 'January',
        'february': 'February',
        'march': 'March',
        'april': 'April',
        'may': 'May',
        'june': 'June',
        'july': 'July',
        'august': 'August',
        'september': 'September',
        'october': 'October',
        'november': 'November',
        'december': 'December',
        'all_years': 'All Years',

        # Group report
        'expense_by_member': 'Expense by Member',
        'expense_by_fund': 'Expense by Fund',
        'member_contribution_summary': 'Member Contribution Summary',
        'fund_summary': 'Fund Summary',

        # Edit group transaction
        'edit_transaction': 'Edit Group Transaction',
        'edited_by': 'Edited by',
        'cancel': 'Cancel',
        'save_changes': 'Save Changes',

        # Split bill
        'logout': 'Logout',
        'from': 'from',
        'paid': 'Paid',
        'not_paid': 'Unpaid',
        'mark_paid': 'Mark as Paid',
        'confirm_delete_bill': 'Are you sure you want to delete this split bill?',
        'delete_bill': 'Delete',
        'custom_amount_placeholder': 'Amount (Rp)',
        'all_paid': 'All paid',
        'pct_paid': 'Percentage paid',
        'total_paid': 'Total paid',
        'total_amount': 'Total bill amount',
        'bill_details': 'Bill Details',
        'split_bill_desc': 'Create a bill split among multiple group members.',
        'bill_title': 'Bill Title',
        'bill_title_placeholder': 'Example: Dinner, Monthly groceries',
        'total_bill': 'Total Bill (Rp)',
        'total_bill_placeholder': 'Example: 150000',
        'split_method': 'Split Method',
        'equal_split': 'Split Equally',
        'custom_split': 'Custom',
        'select_members': 'Select Members',
        'create_split': 'Create Split',
        'split_bills': 'Split Bills',
        'no_split_bills': 'No split bills yet.',
        'created_by': 'Created by',
        'of': 'of',
        'transactions': 'transactions',

        # Settings
        'settings_title': 'Account Settings',
        'profile': 'Profile',
        'preferences': 'Preferences',
        'security': 'Security',
        'export_data': 'Export Data',
        'login_history': 'Login History',
        'delete_account': 'Delete Account',
        'full_name': 'Full Name',
        'email': 'Email',
        'profile_photo': 'Profile Photo (JPG/PNG, max 2MB)',
        'save_profile': 'Save Profile',
        'language': 'Language',
        'theme': 'Theme',
        'light_mode': '☀️ Light Mode',
        'dark_mode': '🌙 Dark Mode',
        'timezone': 'Timezone',
        'notification': 'Notification',
        'notif_popup_label': 'Show popup notifications in app',
        'save_preferences': 'Save Preferences',
        'change_password': 'Change Password',
        'current_password': 'Current Password',
        'new_password': 'New Password',
        'confirm_password': 'Confirm New Password',
        'joined': 'Joined',
        'desc_profile': 'Update your profile information',
        'desc_preferences': 'Customize app appearance and language',
        'desc_security': 'Use a strong and unique password',
        'desc_export': 'Download all your personal transaction data',
        'desc_login_history': 'Your last 5 logins',
        'desc_delete_account': 'This action cannot be undone. All your transactions and budgets will be permanently deleted.',
        'form_delete': '⚠️ This will delete all your transactions, budgets, and account data.',
        'form_preferensi': 'Preferences saved successfully!',
        'export_csv': 'Export CSV',
        'desc_export_csv': 'Universal spreadsheet format, can be opened in Excel / Google Sheets',
        'export_excel': 'Export Excel',
        'desc_export_excel': '.xlsx format with automatic color and formatting',
    }
}

def get_translation(lang='id'):
    return TRANSLATIONS.get(lang, TRANSLATIONS['id'])